...
public abstract class EntityWaterCreature extends EntityBase implements IAnimals {
	...
	public static void inWaterWalk(EntityWaterCreature water, float height, int frequency, float distance)
	{
		if(water.ticksExisted % frequency == 0)
		{
			
			double randRotation = (0.125 - water.rand.nextDouble() * 0.25);
			if(water.ticksExisted % 20 == 0)
			{
				randRotation = (0.5 - water.rand.nextDouble());
			}
			else
			if(water.ticksExisted % 5 == 0)
			{
				randRotation = (0.25 - water.rand.nextDouble() * 0.5);
			}
			water.motionX += (water.LookX() + randRotation) / 10 * distance;
			water.motionY += (water.rand.nextFloat()*4 - 2)/3 * height;
