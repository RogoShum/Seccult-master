	...
	public void getTarget() {
		if(this.doEntity && getEntity() != null) 
		{
			...
			{	
				...
				applyMagickRadius(eList.get(i).world, eList.get(i).posX, eList.get(i).posY + (eList.get(i).height / 2), eList.get(i).posZ, base + addtion);
		...
		if(this.doBlock && getBlock() != null && getEntity() == null)
		{
			...
			for(int i = 0; i < bList.size(); i++)
			{
				if(bList.get(i) != null) {
					...
					applyMagickRadius(player.world, block.getX(), block.getY(), block.getZ(), base + addtion);
